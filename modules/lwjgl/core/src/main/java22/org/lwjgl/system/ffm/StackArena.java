 ...
 */
public class StackArena implements Arena {
    ...
    private static final ThreadLocal<StackArena> TLS = ThreadLocal.withInitial(StackArena::create);
     ...
     */
    protected StackArena(MemorySegment container) {
     ...
     * <p>In the initial state, there is no active stack frame. The {@link #push} method must be used before any allocations.</p>
     */
    public static StackArena create() {
     ...
     */
    public static StackArena create(long capacity) {
    ...
    private static class DebugMemoryStack extends StackArena {
        ...
        public StackArena push() {
            ...
            debugFrames[frameIndex] = stackWalkGetMethod(StackArena.class);
        ...
        @Override
        public StackArena pop() {
            var pushed = debugFrames[frameIndex - 1];
            var popped = stackWalkCheckPop(StackArena.class, pushed);
            if (popped != null) {
    ...
    public MemorySegment callocate(long byteSize) {
        return allocate(byteSize).fill((byte)0);
    }
    ...
    public MemorySegment allocate(long byteSize, long byteAlignment) {
        ...
        var address = (container.address() + pointer - byteSize) & ~(byteAlignment - 1);
    ...
    public MemorySegment callocate(long byteSize, long byteAlignment) {
    ...
    public MemorySegment allocate(MemoryLayout layout) {
        return Arena.super.allocate(layout);
    }

    /** Same as {@link Arena#allocate(MemoryLayout)}. */
    public MemorySegment callocate(MemoryLayout layout) {
        return Arena.super.allocate(layout).fill((byte)0);
    }
    ...
    public MemorySegment allocate(MemoryLayout elementLayout, long count) {
        return Arena.super.allocate(elementLayout, count);
    }

    /** Same as {@link Arena#allocate(MemoryLayout, long)}. */
    public MemorySegment callocate(MemoryLayout layout, long count) {
        return Arena.super.allocate(layout, count).fill((byte)0);
    }
    ...
    public static StackArena stackGet() {
        return TLS.get();
    }
     ...
     */
    public static StackArena stackPush() {
        //noinspection resource
